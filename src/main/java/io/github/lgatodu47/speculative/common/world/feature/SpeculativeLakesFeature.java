...
public class SpeculativeLakesFeature extends Feature<BlockStateFeatureConfig> {
    public SpeculativeLakesFeature(Codec<BlockStateFeatureConfig> codec) {
        super(codec);
        setRegistryName("lakes");
    }
                    ...
                    for (int i3 = 0; i3 < 16; ++i3) {
                        for (int i4 = 0; i4 < 8; ++i4) {
                            if (aboolean[(l1 * 16 + i3) * 8 + i4]) {
                                worldIn.setBlockState(pos.add(l1, i4, i3), i4 >= 4 ? Blocks.CAVE_AIR.getDefaultState() : config.state, 2);
                            }
                        }
                    }
                }
