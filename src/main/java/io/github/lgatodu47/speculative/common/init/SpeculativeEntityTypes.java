...
public class SpeculativeEntityTypes
{
	...
	public static final RegistryObject<EntityType<SpeculoPigEntity>> SPECULO_PIG = ENTITY_TYPES.register("speculo_pig",
			() -> EntityType.Builder.<SpeculoPigEntity>create(SpeculoPigEntity::new, EntityClassification.AMBIENT).size(0.9F, 0.9F).build(new ResourceLocation(Speculative.MODID, "speculo_pig").toString()));
	public static final RegistryObject<EntityType<SpeculoTNTEntity>> SPECULO_TNT = ENTITY_TYPES.register("speculo_tnt",
			() -> EntityType.Builder.<SpeculoTNTEntity>create(SpeculoTNTEntity::new, EntityClassification.MISC).size(0.98F, 0.98F).immuneToFire().build(new ResourceLocation(Speculative.MODID, "speculo_tnt").toString()));

	@SubscribeEvent
	public static void registerAttributes(EntityAttributeCreationEvent event) {
		event.put(SPECULO_PIG.get(), PigEntity.func_234215_eI_().create());
	}
}
