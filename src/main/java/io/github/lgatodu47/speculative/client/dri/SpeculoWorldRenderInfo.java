...
public class SpeculoWorldRenderInfo extends SpeculativeDimensionRenderInfo {
        ...
        if (!(f <= 0.0F)) {
            ...
            for(int renderZ = floorZ - distance; renderZ <= floorZ + distance; ++renderZ) {
                for(int renderX = floorX - distance; renderX <= floorX + distance; ++renderX) {
                    ...
                    double d0 = getRainSizeX(mc.worldRenderer, index) * 0.5D;
                    double d1 = getRainSizeZ(mc.worldRenderer, index) * 0.5D;
                    mutablePos.setPos(renderX, 0, renderZ);
                    Biome biome = world.getBiome(mutablePos);

                    if (biome.getPrecipitation() != Biome.RainType.NONE) {
                        int blockY = world.getHeight(Heightmap.Type.MOTION_BLOCKING, mutablePos).getY();
                        ...
                        if (rainMinY != rainY) {
                            ...
                            mutablePos.setPos(renderX, rainMinY, renderZ);
                            ...
                            if (temperature >= 0.15F) {
                                ...
                                mutablePos.setPos(renderX, l2, renderZ);
                                int j3 = getCombinedLight(world, mutablePos);
                                bufferbuilder.pos((double)renderX - x - d0 + 0.5D, (double)rainY - y, (double)renderZ - z - d1 + 0.5D).tex(0.0F, (float)rainMinY * 0.25F + f3).color(0.302F, 0.165F, 0, f5).lightmap(j3).endVertex();
                                bufferbuilder.pos((double)renderX - x + d0 + 0.5D, (double)rainY - y, (double)renderZ - z + d1 + 0.5D).tex(1.0F, (float)rainMinY * 0.25F + f3).color(0.302F, 0.165F, 0F, f5).lightmap(j3).endVertex();
                                bufferbuilder.pos((double)renderX - x + d0 + 0.5D, (double)rainMinY - y, (double)renderZ - z + d1 + 0.5D).tex(1.0F, (float)rainY * 0.25F + f3).color(0.302F, 0.165F, 0F, f5).lightmap(j3).endVertex();
                                bufferbuilder.pos((double)renderX - x - d0 + 0.5D, (double)rainMinY - y, (double)renderZ - z - d1 + 0.5D).tex(0.0F, (float)rainY * 0.25F + f3).color(0.302F, 0.165F, 0F, f5).lightmap(j3).endVertex();
                            } else {
                                if (i1 != 1) {
                                    if (i1 >= 0) {
                                        tessellator.draw();
                                ...
                                mutablePos.setPos(renderX, l2, renderZ);
                                int k3 = getCombinedLight(world, mutablePos);
                                int l3 = k3 >> 16 & '\uffff';
                                int i4 = (k3 & '\uffff') * 3;
                                int j4 = (l3 * 3 + 240) / 4;
                                int k4 = (i4 * 3 + 240) / 4;
                                bufferbuilder.pos((double)renderX - x - d0 + 0.5D, (double)rainY - y, (double)renderZ - z - d1 + 0.5D).tex(0.0F + f7, (float)rainMinY * 0.25F + f6 + f8).color(1.0F, 1.0F, 1.0F, f10).lightmap(k4, j4).endVertex();
                                bufferbuilder.pos((double)renderX - x + d0 + 0.5D, (double)rainY - y, (double)renderZ - z + d1 + 0.5D).tex(1.0F + f7, (float)rainMinY * 0.25F + f6 + f8).color(1.0F, 1.0F, 1.0F, f10).lightmap(k4, j4).endVertex();
                                bufferbuilder.pos((double)renderX - x + d0 + 0.5D, (double)rainMinY - y, (double)renderZ - z + d1 + 0.5D).tex(1.0F + f7, (float)rainY * 0.25F + f6 + f8).color(1.0F, 1.0F, 1.0F, f10).lightmap(k4, j4).endVertex();
                                bufferbuilder.pos((double)renderX - x - d0 + 0.5D, (double)rainMinY - y, (double)renderZ - z - d1 + 0.5D).tex(0.0F + f7, (float)rainY * 0.25F + f6 + f8).color(1.0F, 1.0F, 1.0F, f10).lightmap(k4, j4).endVertex();
                            }
                        }
                    }
                }
    ...
    private static float getRainSizeX(WorldRenderer renderer, int index) {
        if(RAIN_SIZE_X == null) {
            RAIN_SIZE_X = ObfuscationReflectionHelper.findField(WorldRenderer.class, "field_228411_ak_");
    ...
    private static float getRainSizeZ(WorldRenderer renderer, int index) {
        if(RAIN_SIZE_Z == null) {
            RAIN_SIZE_Z = ObfuscationReflectionHelper.findField(WorldRenderer.class, "field_228412_al_");
